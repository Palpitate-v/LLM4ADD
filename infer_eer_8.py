...
tokenizer = QWenTokenizer.from_pretrained("/root/autodl-tmp/qwen_audio_for_add/models/Qwen-Audio-Chat", trust_remote_code=True)
...
#
#         # 读取 audio_info，但不要修改原始 features
#         audio_infos = [
#             feature.get("audio_info", None)
#             for feature in features
#         ]
#
#         # 给 DataCollatorWithPadding 一份新的字典
#         # 去掉 audio_info，但不影响 Dataset 里的缓存
#         text_features = [
#             {
#                 k: v
...
#
#         if any(info is not None for info in audio_infos):
#
#             audio_span_tokens = []
#             for info in audio_infos:
#                 if info is not None:
#                     audio_span_tokens.extend(
#                         info["audio_span_tokens"]
#                     )
#
#             audio_name_files = []
#             for info in audio_infos:
#                 if info is not None:
#                     audio_name_files.extend(
#                         info["audio_urls"]
#                     )
#
#             batch["audio_info"] = {
...
root_dir = f"./output/temp"
checkpoint_folders = []

for dirname in os.listdir(root_dir):
    checkpoint_dir_path = os.path.join(root_dir, dirname)

    if os.path.isdir(checkpoint_dir_path) and dirname.startswith("checkpoint-"):
...
data_path = "./data/asv_test_fake_audio.jsonl"
test_data = []
...
for peft_path in checkpoint_folders:
    ...
    if os.path.exists(f'./infer_eer/{output_file_name}.json'):
        print(f"文件 {output_file_name} 已存在，跳过操作。")
    else:
        print(f"文件 {output_file_name} 不存在，需要操作。")
        model = QWenLMHeadModel.from_pretrained("/root/autodl-tmp/qwen_audio_for_add/models/Qwen-Audio-Chat", device_map="cuda", trust_remote_code=True).eval()
        peft_model_id = peft_path
        ...
        output_file_name = process_name(peft_model_id)
        with open(f'./infer_eer/{output_file_name}.json','w',encoding='utf-8') as f:
            json.dump(answers,f,indent=4,ensure_ascii=False)
            print(f"已保存推理结果到 ./infer_eer/{output_file_name}.json")
